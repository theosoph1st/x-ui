...
if [[ x"${release}" == x"centos" ]]; then
    if [[ ${os_version} -le 6 ]]; then
        LOGE "Please use CentOS 7+ ！\n" && exit 1
    fi
elif [[ x"${release}" == x"ubuntu" ]]; then
    if [[ ${os_version} -lt 16 ]]; then
        LOGE "Please use Ubuntu 16+ ！\n" && exit 1
    fi
elif [[ x"${release}" == x"debian" ]]; then
    if [[ ${os_version} -lt 8 ]]; then
        LOGE "Please use Debian 8+ ！\n" && exit 1
    fi
fi
...
update() {
    confirm "This function will force the reinstallation of the latest version, the data will not be lost, whether to continue?" "n"
    if [[ $? != 0 ]]; then
        LOGE "Canceled"
        if [[ $# == 0 ]]; then
            before_show_menu
...
uninstall() {
    ...
    echo ""
    echo -e "Uninstall successfully, if you want to delete this script, exit the script and run ${green}rm /usr/bin/x-ui -f${plain} Perform deletion"
    echo ""
...
reset_config() {
    ...
    echo -e "All panel settings have been reset to their default values, now please restart the panel and use the default ${green}54321${plain} Port Access Panel"
    confirm_restart
}
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://raw.githubusercontent.com/theosoph1st/x-ui-en/master/install.sh
    if [[ $? != 0 ]]; then
        echo ""
...
ssl_cert_issue() {
    ...
    LOGI "This script will use the Acme script to apply for a certificate:"
    ...
    LOGI "4.The default installation path for this script to request a certificate is /root/cert directory"
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ $? -ne 0 ]; then
            LOGE "Certificate issuance failure, script exit"
            exit 1
        else
        ...
        if [ $? -ne 0 ]; then
            ...
            LOGI "The certificate is installed successfully, turn on automatic update ..."
        fi
...
show_usage() {
    ...
    echo "x-ui enable       - Set x-ui to boot up automatically"
    echo "x-ui disable      - Cancel x-ui boot-up"
    echo "x-ui log          - View x-ui logs"
    echo "x-ui v2-ui        - Migrate the v2-ui account data from this machine to x-ui"
...
————————————————
  ...
  ${green}3.${plain} Uninstallation x-ui
————————————————
